...
def add_local_layer_coords(hits, jets, layer_radii=LAYER_RADII,
    ...
    relative to its jet's predicted axis-crossing point on that layer.

    use_transverse=True:
      Rotate the global (x, y) by −φ_jet so the jet points along +x. Z dropped.
        local_arc = x·cos φ + y·sin φ   (rotated x — along jet direction)
        local_dz  = −x·sin φ + y·cos φ  (rotated y — perpendicular to jet)
      Hit positions are raw rotated coordinates; φ_jet is no longer needed as
      a model input.

    use_jet_perp=True (default, use_transverse=False):
      Project the 3-D displacement onto the plane perpendicular to the jet
      axis using basis vectors
        ê₁ = (−sin φ,  cos φ,  0)
        ê₂ = (−tanh η·cos φ, −tanh η·sin φ, sech η)
      Stored in local_arc (= x_perp) and local_dz (= y_perp).

    use_jet_perp=False, use_transverse=False:
      Original cylindrical-surface coordinates:
        local_arc = R · Δφ,  local_dz = z_hit − z_axis
    """
    ...
    R   = merged["layer_radius"]
    eta = merged["jet_eta"]
    phi = merged["jet_phi"]

    if use_transverse:
        # rotate global (x, y) by -phi_jet so jet points along +x; keep raw rotated coords
        x = merged["hit_global_x"]
        y = merged["hit_global_y"]
        merged["local_arc"] = x * np.cos(phi) + y * np.sin(phi)
        merged["local_dz"]  = -x * np.sin(phi) + y * np.cos(phi)
    else:
        # displacement from jet-layer intersection point
        dx = merged["hit_global_x"] - R * np.cos(phi)
        dy = merged["hit_global_y"] - R * np.sin(phi)
        dz = merged["hit_global_z"] - R * np.sinh(eta)

        if use_jet_perp:
            sech_eta = 1.0 / np.cosh(eta)
            tanh_eta = np.tanh(eta)
            merged["local_arc"] = -np.sin(phi) * dx + np.cos(phi) * dy
            merged["local_dz"]  = (-tanh_eta * np.cos(phi) * dx
                                    - tanh_eta * np.sin(phi) * dy
                                    + sech_eta * dz)
        else:
            hit_phi = np.arctan2(merged["hit_global_y"], merged["hit_global_x"])
            dphi = wrap_angle(hit_phi - phi)
            merged["local_arc"] = R * dphi
            merged["local_dz"]  = dz
